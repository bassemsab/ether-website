  ...
  try {
    ...
    if (!verifyResult.ok) {
      const errorMap: Record<string, string> = {
        ...
        not_found:
          "Aucun code trouvé ou code déjà utilisé. Veuillez en redemander un.",
        expired:
          "Ce code a expiré (validité 10 minutes). Veuillez en redemander un.",
        too_many_attempts:
          "Nombre maximal de tentatives dépassé. Veuillez redemander un nouveau code.",
      ...
      return json(
        {
          success: false,
          error: errorMap[verifyResult.reason] || "Code invalide",
        },
        { status: 400 },
      );
    }
