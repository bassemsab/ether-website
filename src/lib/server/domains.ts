...
export async function provisionBookedDomain(
  ...
  console.log(`[provisionBookedDomain] Initiating provisioning for ${domain} (${provider}) -> ${forwardToEmail}`);

  const cfToken = env.CLOUDFLARE_API_TOKEN || process.env.CLOUDFLARE_API_TOKEN;
  if (!cfToken) {
    console.warn("[provisionBookedDomain] CLOUDFLARE_API_TOKEN not configured");
    return { success: true, message: `Domain ${domain} simulated provisioning (no CF token)` };
  }
  ...
  try {
    ...
    if (zonesData.result && zonesData.result.length > 0) {
      ...
      const accRes = await fetch("https://api.cloudflare.com/client/v4/zones", { headers: cfHeaders });
      ...
      if (accountId) {
        const createZoneRes = await fetch("https://api.cloudflare.com/client/v4/zones", {
          method: "POST",
          headers: cfHeaders,
          body: JSON.stringify({ name: domain, type: "full", account: { id: accountId } }),
        });
    ...
    if (zoneId) {
      ...
      if (accountId) {
        ...
        await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/email/routing/rules`, {
          method: "POST",
          headers: cfHeaders,
          body: JSON.stringify({
    ...
    return { success: true, message: `Domain ${domain} provisioned and configured successfully.` };
  } catch (err: any) {
